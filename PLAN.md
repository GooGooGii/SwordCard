...
### 高優先（影響玩法完整度）
- [ ] **Boss 專屬神器** — 赤眼山魈、殭屍大帥、拜月教主目前無對應神器（掉隨機遺物），補上
- [ ] **平衡 regression baseline 更新** — 新幕新敵人可能影響 baseline；跑 CI 後確認
- [ ] **更多 PAL1 事件** — 目前 19 種，目標 25+ 種（余杭/蘇州/苗疆/鎖妖塔/拜月各有代表場景）

### 中優先（體驗提升）
- [ ] **Act title 顯示在地圖** — 地圖上方顯示當前幕名稱（余杭山間…）
- [ ] **銅錢差異化** — 各幕 Boss 掉落銅錢量隨幕數提升
- [ ] **牌組視圖** — 分組顯示重複卡；戰鬥中顯示抽/手/棄/消耗堆
- [ ] **卡片出牌動畫** — 從手牌區飛向敵人的簡單 tween
- [ ] **Card Art / 肖像** — 四角色、各幕敵人的正式圖檔（目前為佔位符）
