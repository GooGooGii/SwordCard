...
- **不設出牌張數 / 傷害上限**——引擎組成本身就是難度閘；組成功就讓它爽快地轉到底，不額外砍

### 真・不間斷：牌庫抽乾自動洗牌

- 引擎一旦成立應能**無限運轉**，不做人為截斷（截斷會讓玩家不爽）。
- **抽牌堆抽乾 → 自動洗棄牌堆回抽牌堆**（標準 roguelike-deckbuilder 行為，`DeckManager.draw()` → `_reshuffle_discard()` 已實作，deck_manager.gd:21-50）。
  所以只要引擎淨能量 ≥ 0 且持續抽牌，棄牌堆會一直被洗回來，可一回合打到天荒地老。
- 回能門檻不必設「每 3 張」這種防 turn-1 的限制——薄牌 + 稀有零件已是足夠的成形難度。

### 防呆（純工程，不是數值閘）

- **真死迴圈防護**：唯一要防的是「零輸入無限迴圈」把遊戲卡死。引擎每張牌都由玩家點擊推進、且結算傷害/狀態，回合在「玩家結束 or 敵人死亡」時自然停，不會 UI 凍結。
- **全卡都不在牌堆的邊界**：若手牌 + exhaust 把卡抽光、draw/discard 皆空，`draw()` 安全回傳空（deck_manager.gd:23）不 crash——引擎自然停，但這是極端情形，非人為上限。
- smoke test：寫「理想引擎牌組」案例，驗證**抽牌堆空時正確洗牌續抽、可連續打出大量牌不崩潰**（驗證循環健康，而非設張數上限）。
