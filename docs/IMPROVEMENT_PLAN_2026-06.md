...
**目標**：中段 3%→80% 的升級跳躍壓到「升級是錦上添花，不是及格線」。

| 項 | 內容 |
|---|---|
| 改動點 | `game_data.gd`（各卡 upgrade 數值）、`smoke_test.gd` 升級層 baseline |
| 步驟 | ① 抽 20 張高頻卡列 base→upgrade 增幅表，找出 >+50% 的 outlier → ② 對齊 StS 標準：攻擊卡 +25–33%、技能卡 +33%、能力卡降費優先 → ③ 同步微抬部分 base 值（總強度守恆，曲線變平）→ ④ 重跑升級層兩組 baseline 並更新 |
| 驗證 | smoke 四層全綠；AI 驅動器打 1 場升級流確認手感不疲軟 |
| 風險 | 中。動的卡多，分 2 批 commit，每批跑完整 smoke |
