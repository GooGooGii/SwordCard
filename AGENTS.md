...
角色、敵人、Boss、場景與劇情插圖在繪製／生成前，必須先完成下列步驟：

1. 線上找到並**實際查看**至少一張可清楚辨識造型或場景的 PAL1 原作遊戲圖；優先順序為 DOS／95 版，其次是官方《新仙劍奇俠傳》復刻版。攻略文字不能代替圖片。
2. 向使用者展示參考圖及其來源連結，清楚標示版本，並列出可辨識的輪廓、服裝、兵器、色彩、姿勢或場景元素。
3. 取得使用者確認後，才可撰寫最終 prompt、生成候選圖或修改正式資產。未確認前不得開始繪圖。
4. 新圖以「原作造型的高清重繪」為原則；不得用泛用仙俠、重甲、歐美奇幻或二遊設計取代原作辨識特徵。需要為 UI 調整姿勢／比例時，必須保留原作的核心輪廓與裝束。
5. 候選圖先用新檔名保存並做實機畫面驗收，確認後才替換正式資產。戰鬥實機驗收必須填滿藥品格並加入多件遺物，避免用空白開局 UI 誤判可用空間；Boss 與其意圖可和藥品／遺物區視覺重疊，但文字與操作仍須可讀。

例：苗人頭領應先查看原作／官方復刻版的「胖苗」戰鬥圖，保留黑頭巾、濃鬍、黑紅寬袍與大彎刀；不可自行改成羽冠、盾牌、重甲的泛用苗疆武將。
...
### 敵人肖像朝向（`default_facing_left`）

- `default_facing_left` 描述的是「**原始圖檔是否已面向左側**」，不是要求的最終朝向：原圖朝左設 `true`；原圖朝右、需要在右側戰場面向玩家時設 `false`。
- `UIFactory.ground_portrait()` 會自行調整 `TextureRect` 的尺寸與位置；在此流程中只設定 `TextureRect.flip_h` 曾出現屬性為 `true`、但實機截圖完全沒有翻轉的情況。正式戰鬥 UI 對需要翻轉的肖像改用 `scale.x = -1.0`，並以 `position.x += size.x` 補償原點。
- 朝向修改不可只看旗標或武器位置判斷。必須以同一戰鬥、同一 UI 分別輸出「翻轉／不翻轉」兩張實機 PNG，比對臉部鼻尖、視線、胸口與動勢；兩張若看起來相同即視為翻轉失效。
- 修改資料時必須以敵人的函式或 `enemy.id` 作為 patch 上下文，禁止只替換通用的 `enemy.default_facing_left = ...` 行；2026-07-18 曾因此誤改前方的 `leaf_sprite`，而苗人首領設定實際未變。
