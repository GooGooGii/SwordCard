...
### 🔴 借圖待補（3 張，2026-06-09 新增）
目前新增了 3 張為引擎核心的罕見/傳奇卡牌，暫時處於借圖狀態，待額度恢復後需補齊專屬去背水墨圖檔：
- `lxy_yujianxinjue` (御劍心訣) ← 借 `lxy_yujian` (御劍術)
- `zl_lingxijue` (靈息訣) ← 借 `zl_guanyin` (觀音符)
- `zl_wanlingshi` (萬靈噬) ← 借 `zl_diliebeng` (地裂天崩)
