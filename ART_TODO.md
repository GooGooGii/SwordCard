...
> **目前優先順序**（2026-07-15 更新）：
> 0. ~~§18 戰鬥背景明度重生~~（2026-07-17 成品否決、四張已回退，**擱置**——重啟條件見 §18 結案紀錄）
> 0.5 **§19 赤鬼王正史對位重繪 ×2（＋劇情圖低優先）**（製作人比對 PAL1 截圖裁決：現圖是日式紅鬼，正史是黑袍高冠隋將屍官）——注意：§6C 的 `tomb_general` phase-2 落檔工作**併入本項**，勿按舊概念圖落檔
> 1. §6C Boss phase 2 圖 ×2 落檔（`tomb_general` / `centipede_lord`，概念圖已生成）——`tomb_general` 部分改走 §19
...
- **主選單題字**（`assets/art/ui/title_swordcard.png`）：🟢 已完成 (2026-07-09 重製)。按青銅毛筆風格重繪，包含「劍」字與「SwordCard」字樣，並做去背與深色描邊優化，確保淺色背景下的可讀性。
...
| ID | 卡名 | 問題 | 優先 |
|---|---|---|---|
| `lxy_yujian` | 御劍術（**起始卡×3，曝光最高**）| 🟢 已完成專屬水墨圖 (2026-07-09 重製) | — |
...
## 十六、新遺物圖示（🟢 11 件已全部完成，2026-07-09 遺物設計優化批次）
...
| id | 名稱 | 類型 | 狀態 / 建議意象 |
|---|---|---|---|
| `mu_jian` | 木劍 | 李逍遙 common 專武 | 🟢 已完成（樸拙木劍，淡棕木紋，少年習劍的第一把劍） |
| `yuenv_jian` | 越女劍 | 趙靈兒 common 專武 | 🟢 已完成（纖細古劍、青碧劍穗，靈氣繚繞水色調） |
| `linjia_jiansui` | 林家劍穗 | 林月如 common 專武 | 🟢 已完成（赤紅劍穗流蘇結飾，英氣俠女佩飾） |
| `baigu_nang` | 百蠱囊 | 阿奴 common 專武 | 🟢 已完成（苗銀紋繡布囊、微露蟲影，墨綠底苗疆風） |
| `kaishan_fu` | 開山符 | common 通用 | 🟢 已完成（硃砂「開山」符籙，金橙色劈石氣勁） |
| `poyao_sha` | 破妖砂 | common 通用 | 🟢 已完成（掌中一撮金砂揚出，破邪光點） |
| `tongqian_jian` | 銅錢劍 | common 通用 | 🟢 已完成（紅繩串銅錢成劍形，民俗法器） |
| `yu_puti_zhu` | 玉菩提珠 | rare 取捨遺物 | 🟢 已完成（溫潤玉質菩提念珠，金光內斂帶一絲佛威） |
| `kezhan_yaopai` | 客棧腰牌 | uncommon run 層 | 🟢 已完成（木製客棧腰牌刻紋，繫繩） |
| `qiankun_dai` | 乾坤袋 | uncommon run 層 | 🟢 已完成（鼓脹錦囊、銅錢隱現，袋口束繩金線） |
| `xingshen_cha` | 醒神茶 | uncommon run 層 | 🟢 已完成（青瓷茶盞、裊裊熱氣凝成靈光） |

### 十六之二、Boss 池代價神器圖示（🟢 3 件已全部完成，2026-07-09 Batch C1）
...
## 十七、商店畫面美術（🟢 已全部完成，2026-07-09 商店美化審查）

實機渲染審查結論：商店目前是「深藍面板＋文字清單」，山道商店與夜路黑店除標題外**視覺完全相同**，
無場景感、無商人角色、無店鋪氛圍。以下美術補齊後由程式接入（接入屬另一工單，本節只管圖）。

| 檔名 | 用途 | 規格 | 狀態 / 建議意象 |
|---|---|---|---|
| `assets/art/shop_bg.png` | 山道商店背景（取代現借用的 `event_bg.png`） | 16:9 滿版、**不去背** | 🟢 已完成（山道旁行商小攤，暖褐＋米黃水墨調） |
| `assets/art/shop_black_bg.png` | 夜路黑店背景 | 16:9 滿版、**不去背** | 🟢 已完成（夜色小屋簾幕半掩，紫黑＋暗金調） |
| `assets/art/npc/shop_merchant.png` | 山道商店行商立繪 | 去背 RGBA、半身、主體居中 | 🟢 已完成（憨厚中年行商，笑瞇瞇撥算盤） |
| `assets/art/npc/shop_black_merchant.png` | 黑店老闆立繪 | 去背 RGBA、半身、主體居中 | 🟢 已完成 (2026-07-15 重製)（簾影中的乾瘦掌櫃，指節細長捻著銅錢） |

> 立繪顯示尺寸約 220–300px 高（標題旁／面板側），畫 512–768px 即可。
> 其餘商店美化（宣紙底紋、卷軸邊框、區段毛筆分隔線、特賣印章）**沿用既有資源**
> （`paper_texture.png` / `scroll_frame.png` / `brush_divider.png`），屬程式接入，不需新圖。

---

## 十八、戰鬥背景明度重生（❌ 成品否決、已回退，2026-07-17 製作人裁決）

**背景**：2026-07 盤點確認全部 8 張幕背景＋主選單皆為低明度 dark-fantasy，開場 act_1 讀起來像恐怖遊戲，違背 PAL1「前中期白天暖陽田園、陰暗是後期轉折」的基調。裁決與完整規範見 `assets/art/ART_GUIDE.md`「明度紀律」＋「背景 Prompt Pattern（2026-07 明度重校批次）」。已全部完成日光背景重校。

| 檔名 | 場景 | 明度目標 | 狀態 |
|---|---|---|---|
| `assets/art/battle_bg_act_1.png` | 餘杭/十里坡：晨光竹林山道＋小亭 | ☀ 亮（晨光田園、清爽有希望感） | ❌ 重製版否決，已回退舊圖 (2026-07-17) |
| `assets/art/battle_bg_act_5.png` | 隱龍窟：晶光通明的靈氣洞窟 | ☀ 亮 | ❌ 重製版否決，已回退舊圖 (2026-07-17) |
| `assets/art/battle_bg_act_7.png` | 苗疆：白日山寨、陽光瀑布、綠意彩旗 | ☀ 亮 | ❌ 重製版否決，已回退舊圖 (2026-07-17) |
| `assets/art/main_menu_bg.png` | 主選單：同構圖改晨曦仙山 | ☀ 亮 | ❌ 重製版否決，已回退舊圖 (2026-07-17) |
| act_2 | 蘇州水鄉 | ☀ 已達標 | ✅ 保留——全套明度基準 |
| act_3 / 4 / 6 / 8 | 鬼森／將軍塚／boss 殿／終局祭壇 | 🌙 暗（正史合理） | ✅ 保留 |

> 規格：1280×720（16:9）滿版**不去背** PNG、無文字。舊圖直接覆蓋同檔名，刪舊 `.import` 後
> `godot --headless --path . --import` 重匯入即生效（程式以路徑載圖）。

> **結案紀錄（2026-07-17）**：技術驗證雖通過（實機截圖 UI 可讀、smoke passed），但**製作人看過成品後否決整批**——
> 重繪版執行風格偏童話/anime 水彩，與遊戲其餘畫面（act_2 基準與各單體資產的水墨厚塗質感）違和。
> 四張圖已 `git checkout` 回退至重繪前版本（act_1 於 `caf6f17^`、其餘三張於 `d5eae9e^`；重繪版仍留在
> 這兩個 commit 的歷史中可查）。同批的赤鬼王重繪（`dd501c5`）**不在否決範圍，保留**。
> **教訓**：明度（要不要亮）與畫風（怎麼畫亮）是兩個獨立問題——這批解了前者、砸了後者。
> 若之後重啟，先用 act_1 單張對齊「act_2 的 painterly 質感＋日光明度」再批量，勿再用 storybook 水彩方向。

## 十九、赤鬼王正史對位重繪（🟢 已全部完成，2026-07-15 製作人回報）

製作人比對 PAL1 原作截圖後裁決：現行 `tomb_general` 三張圖已依據正史重繪。
**PAL1 正史形象**：被妖靈操控的隋朝將軍屍身——蒼白面容、黑髯、額間赤色印記、黑色官袍、高冠、半身浸於血池。已全部完成重繪。

| 檔名 | 用途 | 規格 | 狀態 / 建議意象 |
|---|---|---|---|
| `assets/art/enemies/tomb_general.png` | 一階肖像 | 去背 RGBA、面向左 | 🟢 已完成 (2026-07-15 重製)（黑袍高冠、下半身沒入血池之屍官） |
| `assets/art/enemies/tomb_general_phase2.png` | 二階肖像 | 去背 RGBA、面向左 | 🟢 已完成 (2026-07-15 重製)（沿用官袍高冠輪廓、體內湧出血焰） |
| `assets/art/story/tomb_general.png` | 劇情擊敗圖 | 16:9 滿版不去背 | 🟢 已完成 (2026-07-15 重製)（血池退去乾涸、將軍安息、土靈珠發光） |

> 參考：PAL1 原作赤鬼王戰鬥畫面＝黑袍高帽官人立於全紅血池中。正史考據（血池/黑水鎮殭屍/土靈珠）
> 見 `docs/PAL1_CANON.md` §四（2026-07-10 網路查證）。
