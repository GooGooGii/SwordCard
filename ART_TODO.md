...
> 0. **§18 戰鬥背景明度重生 ×4＋主選單**（製作人裁決：全套太暗、違背 PAL1 基調）——**首張 `act_1` 為驗證張，最高優先**
...
## 十八、戰鬥背景明度重生（🔴 2026-07-15 製作人裁決，最高優先）

**背景**：2026-07 盤點確認全部 8 張幕背景＋主選單皆為低明度 dark-fantasy，開場 act_1 讀起來像恐怖遊戲，違背 PAL1「前中期白天暖陽田園、陰暗是後期轉折」的基調。裁決與完整規範見 `assets/art/ART_GUIDE.md`「明度紀律」＋「背景 Prompt Pattern（2026-07 明度重校批次）」（**內含 act_1 可直接複用的完整 prompt**）。

**流程**：先只出 `act_1` 一張 → 接入 → `render_battle_backgrounds.gd` 實機截圖肉眼驗證新方向 → 通過後再批量出其餘。

| 檔名 | 場景 | 明度目標 | 狀態 |
|---|---|---|---|
| `assets/art/battle_bg_act_1.png` | 餘杭/十里坡：晨光竹林山道＋小亭 | ☀ 亮（晨光田園、清爽有希望感） | 🔴 **驗證張，先出這張** |
| `assets/art/battle_bg_act_5.png` | 隱龍窟：晶光通明的靈氣洞窟（順手改 16:9，現為 3:2） | ☀ 亮 | ⬜ 待 act_1 驗證通過 |
| `assets/art/battle_bg_act_7.png` | 苗疆：白日山寨、陽光瀑布、綠意彩旗 | ☀ 亮 | ⬜ 待 act_1 驗證通過 |
| `assets/art/main_menu_bg.png` | 主選單：同構圖改晨曦仙山 | ☀ 亮 | ⬜ 待 act_1 驗證通過 |
...
## 十九、赤鬼王正史對位重繪（🔴 2026-07-15 製作人回報：與 PAL1 形象嚴重不符）

製作人比對 PAL1 原作截圖後裁決：現行 `tomb_general` 三張圖**沒有一張是正史的赤鬼王**——
現一階圖是日式紅皮赤鬼（獠牙鬼角骷髏大劍）、二階是妖化骷髏王、劇情圖是青綠幽魂將軍（改名前
「塚中亡將」舊概念）。**PAL1 正史形象**（見 `docs/PAL1_CANON.md` §四；原作將軍塚血池 boss）：
**被妖靈操控的隋朝將軍屍身**——蒼白灰敗面容、黑髯、額間赤色印記、黑色官袍、高冠（襆頭式高帽）、
半身浸於血池，通體是「屍官陰森」而非「妖鬼兇猛」。

| 檔名 | 現況問題 | 重繪方向 | 優先 |
|---|---|---|---|
| `assets/art/enemies/tomb_general.png` | 紅皮日式赤鬼，完全跑偏 | 正史屍官：黑袍高冠、蒼白蓄鬚、額間赤印，下半身沒入暗紅血池、血氣繚繞；去背水墨、面向左（`portrait_scale 1.4`） | 高 |
| `assets/art/enemies/tomb_general_phase2.png` | 妖化骷髏王，與一階無連續性 | 血池妖力全開：**延續一階的官袍/高冠輪廓**（玩家要認得是同一隻），屍身崩裂、妖靈血焰自體內湧出、血池翻騰滔天；可沿用現圖的血焰語彙 | 高 |
| `assets/art/story/tomb_general.png` | 青綠幽魂將軍（舊概念殘留），且無血池 | 擊敗後：血池退去乾涸、將軍屍身歸於安息、土靈珠微光（正史掉落物彩蛋）；16:9 滿版不去背 | 低 |
