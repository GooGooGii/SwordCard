...
static func main_menu_button(text: String, emphasized: bool = false) -> Button:
	...
	button.add_theme_stylebox_override("normal", style_box(base, border, 2, 10))
	button.add_theme_stylebox_override("hover", style_box(hover, border.lightened(0.08), 2, 10))
	button.add_theme_stylebox_override("pressed", style_box(pressed, border, 2, 10))
