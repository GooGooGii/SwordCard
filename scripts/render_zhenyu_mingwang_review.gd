...
const CANDIDATE_BG := "res://assets/art/battle_bg_boss_zhenyu_mingwang_v3.png"
const CANDIDATE_PORTRAIT := "res://assets/art/enemies/zhenyu_mingwang_pal1_v3.png"
...
func _initialize() -> void:
	...
	main.run_state.act = 6
	main.run_state.act_intro_seen = 6

	var boss: EnemyData = GameData.boss_for_act(6).clone()
	boss.portrait_path = CANDIDATE_PORTRAIT
	boss.portrait_scale = 1.9
	boss.default_facing_left = true
	main.start_next_battle(boss)
	await process_frame
	await process_frame
	main._set_background(CANDIDATE_BG)
	await create_timer(1.5).timeout
	_save_viewport(main, "%s/zhenyu_mingwang_candidate_facing_left.png" % abs_dir)
	...
	await process_frame
	_save_viewport(main, "%s/zhenyu_mingwang_candidate_facing_right.png" % abs_dir)
