...
func _test_save_round_trip(characters: Array[CharacterData]) -> void:
	...
	state.current_shop_relic_ids = ["tongbao_qian", "lie_huo_ling"]
	...
	_check(restored.current_shop_relic_ids.size() == 2 and restored.current_shop_relic_ids[0] == "tongbao_qian", "current_shop_relic_ids mismatch")
...
const BALANCE_BASELINES_LEVELED: Dictionary = {
	...
	# 2026-06-11 趙靈兒 leveled 谷底修正（故意調整）：解鎖軌直傷上修
	...
	# 2026-06-30 角色公平性修正（故意調整）：阿奴全分級檔位 100%、無敗場——蠱毒繞護體 + 終幕
	# 無抗毒剋星。把鎖妖塔 boss 鎮獄明王（act 6 / Lv15）立為「毒流之牆」：兩個疊盾招式 金剛法相 /
	# 明王金身 加 absorb_poison（噬毒化力）。此 kind 只吃蠱毒、對非毒流角色為 no-op，故李/趙/林
	# Lv15 完全不變（delta 0）；阿奴 Lv15 100→87（毒流終於有一場真硬仗；burst 仍能贏故非歸零）。
	# 終幕拜月教主維持純戰力檢定（試過加抗毒，但阿奴靠 poison_burst 在敵回合前引爆、反應式抗毒無效，
